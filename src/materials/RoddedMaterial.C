...
validParams<RoddedMaterial>()
{
  ...
  params.addRequiredParam<Real>("rodPosition", "Position of the control rod.");
  params.declareControllable("rodPosition");
  ...
  params.addParam<MooseEnum>("rodDimension", validDims, "Dimension rod is parallel to. x, y, or z");
  return params;
}
...
RoddedMaterial::RoddedMaterial(const InputParameters & parameters)
  : GenericMoltresMaterial(parameters),
    ...
    _rod_pos(getParam<Real>("rodPosition")),
...
RoddedMaterial::computeQpProperties()
{
  ...
  if (_q_point[_qp](_rod_dim) < _rod_pos)
